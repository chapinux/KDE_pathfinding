...
adjacency_list_simple <- lapply(adjacency_list, function(neighbors) {
  if (is.null(neighbors)) {
    return(NULL)
  } else {
    return(sapply(neighbors, function(x) x$node))
  }
})

# Créer le graphe non orienté
g <- graph_from_adj_list(adjacency_list_simple)

# Fonction pour récupérer le coût entre deux nœuds
# Fonction pour calculer la distance entre deux noeuds (diagonale ou non)
calculate_distance <- function(node1, node2, grid_cols) {
  coords1 <- get_coordinates(node1, grid_cols)
  coords2 <- get_coordinates(node2, grid_cols)
  
  # Calcul de la différence sur les axes ligne et colonne
  row_diff <- abs(coords1[1] - coords2[1])
  col_diff <- abs(coords1[2] - coords2[2])
  
  # Si les deux différences sont 1, alors c'est une diagonale
  if (row_diff == 1 && col_diff == 1) {
    return(sqrt(2))  # Déplacement diagonal
  } else if (row_diff == 1 || col_diff == 1) {
    return(1)  # Déplacement horizontal ou vertical
  } else {
    return(0)  # Pas de mouvement (même noeud)
  }
}

# Fonction pour calculer le coût total du chemin
calculate_path_cost <- function(path, grid_cols) {
  total_cost <- 0
  for (i in 1:(length(path) - 1)) {
    total_cost <- total_cost + calculate_distance(path[i], path[i + 1], grid_cols)
  }
  return(total_cost)
}
...
start_time <- Sys.time()
resultat_djikstra <- shortest_paths(g, from = start_node, to = goal_node)
end_time <- Sys.time()
execution_time <- as.numeric(difftime(end_time, start_time, units = "secs"))

# Calcul du coût total pour le chemin trouvé par Dijkstra
cout_total <- calculate_path_cost(resultat_djikstra$vpath[[1]], grid_cols)

cat("Chemin Dijkstra:", resultat_djikstra$vpath[[1]], "\n")
cat("Coût Dijkstra :", cout_total, "\n")
cat("temps d'exécution Djikstra :", execution_time, "\n")


# Affichage
#plot_grid(grid_rows, grid_cols, adjacency_list, start_node, goal_node, result$path, obstacles)

#plot_grid(grid_rows, grid_cols, adjacency_list, start_node, goal_node, resultat_djikstra$vpath[[1]], obstacles)
...
for (test_case in test_cases) {
  ...
  for (j in 1:100) {
    ...
    results <- rbind(results, data.frame(Algorithm="Djikstra" ,ID_paire=paste("P", i, sep = ""),ID_rep=paste("R", j, sep = ""),Start = start, End = end, ExecutionTime = execution_time,cout=cout_total))
    cout_total<-0
...
for (test_case in test_cases) {
  ...
  for (j in 1:100) {
    start_time <- Sys.time()
    result <- a_star(adjacency_list, euclidean_heuristic, start_node, goal_node, grid_cols)
    cout_total=result$cost
    end_time <- Sys.time()
    execution_time <- as.numeric(difftime(end_time, start_time, units = "secs"))
    result <- NULL
    
    # Stocker les résultats dans le tableau
    results <- rbind(results, data.frame(Algorithm="A*",ID_paire=paste("P", i, sep = ""),ID_rep=paste("R", j, sep = "") ,Start = start, End = end, ExecutionTime = execution_time,cout=cout_total))
    cout_total<-0
    }
  i<-i+1
}
...
for (test_case in test_cases) {
  ...
  for (j in 1:100) {
    ...
    # Stocker les résultats dans le tableau
    results <- rbind(results, data.frame(Algorithm="JPS",ID_paire=paste("P", i, sep = ""),ID_rep=paste("R", j, sep = "") ,Start = start, End = end, ExecutionTime = execution_time,cout=cout_total))
  }
  i<-i+1
}
...
write.xlsx(results, "execution_times2.xlsx")
...
results <- read.xlsx("execution_times2.xlsx")
...
saveWorkbook(wb, "resultats_avec_resume4.xlsx", overwrite = TRUE)
